...
const AudioPreview = ({
  ...
  const telegramAudio = useRef(null);
  const telegramAudioTime = useRef(null);
  ...
  const timeUpdateFunc = (e) => {
    ...
    telegramAudioTime.current.innerHTML = `${
    ...
    if (currentPlayState && e.target.ended) {
      ...
      telegramAudioTime.current.innerHTML = `${totalMin}:${
        totalSec < 10 ? `0${totalSec}` : totalSec
      }`;
    }
  };
  ...
  const clickHandler = () => {
    ...
    if (telegramAudio.current !== null) {
      // 객체 null 예외처리
      if (!currentPlayState) {
        // 상태 변화 전 pause였으면
        telegramAudio.current.play();
      } else {
        // 상태 변화 전 play 였으면
        telegramAudio.current.pause();
      }
    }
  };
      ...
      >
        <div className="audio-container">
          {currentPlayState && v.content !== "" ? (
            <i
              className="fas fa-pause audio-icon-telegram"
              onClick={clickHandler}
            ></i>
          ) : (
            <i
              className="fas fa-play audio-icon-telegram"
              onClick={clickHandler}
            ></i>
          )}
          <div className="audio-content-telegram">
            ...
            <div className="audio-size" ref={telegramAudioTime}>

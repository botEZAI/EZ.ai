...
const AudioPreview = ({
  ...
  const timeUpdateFunc = (e) => {
    ...
    telegramAudioTime.current.innerHTML = `${minutes < 10 ?
    ...
    if(currentPlayState && e.target.ended) {
      ...
      telegramAudioTime.current.innerHTML = `${totalMin}:${totalSec < 10 ?`0${totalSec}` : totalSec}`
    }
  }
  ...
  const clickHandler = () => {
    ...
    if(telegramAudio.current !== null) { // 객체 null 예외처리
